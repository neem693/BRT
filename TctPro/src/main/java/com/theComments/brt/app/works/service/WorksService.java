...
//		data.put("work", worksDto);
	 ...
	 */
	public ResultMap selectWorksByArtist(Long id) {
		// TODO Auto-generated method stub
...
//		int size = PageConst.PAGE.PAGE_SIZE_NINE;
		...
		map.put("see", resultListSee);
		map.put("seeCount", seeCount);

		map.put("listen", resultListListen);
		map.put("listenCount", listenCount);

		map.put("doo", resultListDoo);
		map.put("dooCount", dooCount);

		map.put("artist", artistOne.get());
